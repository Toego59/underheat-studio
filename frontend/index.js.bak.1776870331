...
// API endpoints
const API_LOGIN = 'https://underheat-studio.onrender.com/api/login';
const API_REGISTER = 'https://underheat-studio.onrender.com/api/register';
...
// Debug logging (admin-only panel)
...
document.addEventListener('keydown', (e) => {
  if (e.key === '`' && e.shiftKey) {
    if (!currentIsAdmin) {
      console.log('Debug panel blocked: user is not admin');
      return;
    }
...
function setAuthState(auth, username = null, isAdmin = false) {
  ...
  // Hide debug panel if user is not admin
  if (!isAdmin) {
...
async function loginViaApi() {
  ...
  try {
    ...
    if (!res.ok || !data.success) {
      ...
      log('Login failed: ' + (data.message || res.status));
      return;
    }
...
async function registerViaApi() {
  ...
  try {
    ...
    if (!res.ok || !data.success) {
      ...
      log('Registration failed: ' + (data.message || res.status));
      return;
    }
...
async function toggleWebamp() {
  if (!isAuthenticated || !currentIsAdmin) {
    ...
    log('Non-admin attempted Webamp access');
    return;
  }
...
function setTheme(theme) {
  ...
  localStorage.setItem('--primary-color', getComputedStyle(root).getPropertyValue('--primary-color').trim());
  localStorage.setItem('--secondary-color', getComputedStyle(root).getPropertyValue('--secondary-color').trim());
  localStorage.setItem('--accent-color', getComputedStyle(root).getPropertyValue('--accent-color').trim());
  localStorage.setItem('--background-color', getComputedStyle(root).getPropertyValue('--background-color').trim());
...
// Customization: background, cards, font, scale
function applyCustomizations() {
  ...
  if (bgImagePicker) bgImagePicker.classList.toggle('hidden', bgStyle !== 'image');
}
...
function wireCustomizationControls() {
  ...
  if (bgStyleSelect) {
    bgStyleSelect.addEventListener('change', (e) => {
      ...
      if (bgImagePicker) bgImagePicker.classList.toggle('hidden', value !== 'image');
      log('Background style: ' + value);
    });
  }
...
window.addEventListener('load', () => {
  ...
  if (webampToggle) {
    webampToggle.addEventListener('click', toggleWebamp);
  }
